...
from config import INVITE_DELAY, API_ID, API_HASH

async def get_client(phone):
    return TelegramClient(f"session_{phone}", API_ID, API_HASH)

async def join_chat(phone, chat_link):
...
async def parse_chat(phone, chat_link, progress_callback=None):
    client = await get_client(phone)
    await client.start()
    
    try:
        entity, is_member = await join_chat(phone, chat_link)
        if not is_member:
            print("Успешно вступил в цель")
        
        count = 0
        async for user in client.iter_participants(entity):
            phone_user = user.phone if hasattr(user, "phone") else None
            save_user(user.username, user.id, user.first_name, phone_user)
            count += 1
...
async def invite_users(phone, target_link, progress_callback=None):
    client = await get_client(phone)
    await client.start()
    
    try:
        target, is_member = await join_chat(phone, target_link)
        if not is_member:
        ...
        client2 = await get_client(phone)
        await client2.start()
