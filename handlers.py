...
from keyboards import main_reply_kb, after_parse_kb, cancel_kb, retry_auth_kb
...
async def reset_cmd(msg: types.Message):
    ...
    if os.path.exists(session_file):
        ...
        await msg.answer("✅ Сессия удалена. Теперь авторизуйтесь заново через /auth", reply_markup=main_reply_kb())
    else:
        await msg.answer("❌ Сессия не найдена.", reply_markup=main_reply_kb())
...
async def process_code(msg: types.Message, state: FSMContext):
    ...
    if not success:
        if "устарел" in result or "expired" in result:
            await msg.answer(
                f"❌ {result}\n\n"
                "Нажмите '🔄 Запросить новый код', чтобы получить свежий код.",
                reply_markup=retry_auth_kb()
            )
            return
...
async def retry_auth(call: types.CallbackQuery, state: FSMContext):
    await call.message.delete()
    phone = user_phone.get(call.from_user.id)
    if not phone:
        await call.message.answer("❌ Сессия потеряна. Начните заново через /auth")
        await state.clear()
        await call.answer()
        return
    
    await call.message.answer("⏳ Отправляю новый код...")
    success, result = await send_code(phone)
    if not success:
        await call.message.answer(f"❌ {result}", reply_markup=main_reply_kb())
        await state.clear()
        await call.answer()
        return
